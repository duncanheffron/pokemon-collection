#!/usr/bin/env node
/**
 * Scrape Miltank-related cards from PriceCharting
 */
const puppeteer = require('puppeteer');
const fs = require('fs');

const searchQueries = [
  { name: 'Miltank', url: 'https://www.pricecharting.com/search-products?q=miltank&type=prices' }
];
...
async function main() {
  console.log('Starting Miltank Collection scrape...\n');
  ...
  // Save raw data
  fs.writeFileSync('miltank_collection_raw.json', JSON.stringify(uniqueCards, null, 2));
  console.log('✓ Saved raw data to miltank_collection_raw.json');
  
  // Process into set format
  processMiltankCollection(uniqueCards);
}

function processMiltankCollection(cards) {
  ...
  cards.forEach(card => {
    ...
    if (originalTitle.includes('[Ball]')) {
      ...
      variantType = 'V-Union';
    } else if (originalTitle.includes('[Mirror Holo]') || originalTitle.includes('Mirror Holo')) {
      variantType = 'Mirror Holo';
    }
  ...
  // Create the set
  const miltankSet = {
    id: 'miltank-collection',
    name: 'Miltank Collection',
  ...
  // Remove existing Miltank Collection if present
  allSets = allSets.filter(s => s.id !== 'miltank-collection');
  
  // Add new set
  allSets.push(miltankSet);
  
  // Save
  fs.writeFileSync('data/sets.json', JSON.stringify(allSets, null, 2));
  console.log(`✓ Created "Miltank Collection" set with ${processedCards.length} unique cards`);
